...
select IDDISCIPLINA, format (avg(NOTA), 1) as 'média da nota' from BOLETIM group by IDDISCIPLINA having avg(NOTA) > 6 order by avg(NOTA);
...
select PRONTUARIO, format (avg(NOTA), 1) as 'médias notas' from BOLETIM group by PRONTUARIO having avg(NOTA) > 7 order by avg(NOTA) desc;
...
select PRONTUARIO, IDDISCIPLINA, min(NOTA) as 'menor nota' from BOLETIM group by PRONTUARIO, IDDISCIPLINA;

#20. Exibir o prontuário, a identificação da disciplina e a nota das 2 maiores notas registradas no boletim. 
select PRONTUARIO, IDDISCIPLINA, max(NOTA) from BOLETIM group by PRONTUARIO, IDDISCIPLINA limit 2;
